    ...
    if single:
        ...
        for key in base:
            if len(base[key]) != 0:
                ...
                # print(data_list)
                ans.extend(list(map(frozenset,create_FP_tree(data_list,min_support,key))))
...
def judege(dataset,sets,min_support):
    dicts = {}
    lengths = len(dataset)
    # print(lengths)
    for item in sets:
        for tran in dataset:
            if frozenset(tran).issubset(item):
                # print(frozenset(tran))
                if dicts.get(item,0) == 0:
                    dicts[item] = 1
                else:
                    dicts[item] += 1
    ans = []
    ...
    for key in dicts:
        if dicts[key] >= lengths * min_support:
            ans.append(key)
    return ans


if __name__ == '__main__':
    data_path = 'retail.dat'
    ...
    dataset = data[:int(length * rate)]
    total = len(dataset)
    # print(total)
    ans = FP_growth(min_support,dataset)
    set1 = set(ans)
    print(len(set1))
    ...
    ans = []
    ans.extend(c1)
    ans.extend(judege(data,c3,min_support))
    ans.extend(judege(data,c2,min_support))
    ans = set(ans)
    # print(ans)
    print(len(ans))

    # print(all_dict_1)
    # data2 = data[int(length*rate):]
    # print(ans1)